...
module curve_strut(length, width, sagitta, dir, pivot_r) {
  // render() {
    union() {
      #difference() {
        cylinder(r=pivot_r*1.5, h=material_thickness, center=true);
      ...
      intersection() {
        ...
        difference() {
          ...
          cylinder(r=pivot_r*1.5, h=material_thickness*2.1, center=true);
          translate([length, 0, 0]) cylinder(r=pivot_r*1.5, h=material_thickness*2.1, center=true);
        }
      }
    }
  // }
  translate([0, 0, dir * (material_thickness+1)]) {
