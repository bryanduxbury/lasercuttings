// TODOs
// switch gear ratio to 20:8
// calculate mm_per_tooth based on 
// design the retaining disc
// design the partial pinion
// redesign connecting gear spacer (alignment helper for gear stacks)
// design drum spacers (alignment helper for gear stacks)
// lengthen the tabs on numeral plates to serve as alignment helpers
// add new side plates to numeral drums
...
// minimum thickness we'll allow
gear_minimum_width = 10;
...
numeral_circumference = 10 * (numeral_h + numeral_spacing);
outside_diameter = numeral_circumference / 3.141592;
mm_per_tooth = 3.141592 / (22 / outside_diameter);

// some functionalized parameters
function numeral_radius() = outside_diameter / 2;
function drum_width() = numeral_w + 2*t + 2 * washer_t;

// case 
///////////////

// space between guts and the external case
case_clearance = 12.5;

case_width = 9 * drum_width() + 10 * t + 2 * case_clearance + motor_body_length + motor_nonconnecting_shaft_length;
case_height = outside_diameter + 2*case_clearance;
case_depth = case_height;

// let's print out everything we need to know to customize parts and buy materials!
///////////////////
...
module connecting_gear() {
  ...
  difference() {
    ...
    translate([0, pitch_radius(number_of_teeth=8, mm_per_tooth=mm_per_tooth), 0]) cylinder(r=0.5, h=t*2, center=true);
  }
}
...
module drum_gear_cutouts() {
  assign(or = root_radius(number_of_teeth = 20, mm_per_tooth = mm_per_tooth) - gear_minimum_width) difference()  {
...
module drum_partial_gear() {
  render() difference() {
    union() {
      rotate([0, 0, -360/20 * 3]) my_gear(number_of_teeth = 20, teeth_to_hide=18);
      cylinder(r=root_radius(number_of_teeth=20,mm_per_tooth=mm_per_tooth), h=t, center=true, $fn=128);
    }
...
module drum_support() {
  ...
  render() 
  assign(h=outer_radius(number_of_teeth = 20, mm_per_tooth=mm_per_tooth)+case_clearance)
  difference() {
    union() {
...
module connecting_gear_support() {
  color([128/255, 0/255, 0/255]) 
  render()
  assign(cd = center_distance(8, 20))
  assign(pr1 = pitch_radius(number_of_teeth=20, mm_per_tooth=mm_per_tooth))
  assign(or1 = outer_radius(number_of_teeth=20, mm_per_tooth=mm_per_tooth))
...
module drum_retaining_disc() {
  color([255/255, 128/255, 64/255])
  cylinder(r=numeral_radius() + 1.5 * t, h=t, center=true, $fn=72);
}

module drum_assembly() {
  for (i=[-1,1]) {
    translate([i * (numeral_w/2 + t/2), 0, 0]) rotate([0, 90, 0]) drum_retaining_disc();
  }

  translate([numeral_w/2 + t + t/2, 0, 0]) rotate([0, 90, 0]) drum_complete_gear();
  translate([-numeral_w/2 - t - t/2, 0, 0]) rotate([0, 90, 0]) drum_partial_gear();
...
module bottom() {
  ...
  difference() {
    ...
    assign(yoff = center_distance(20, 8) * sin(45))
...
module mock_motor() {
  ...
  color([192/255, 192/255, 192/255]) translate([0, 0, 4]) scale([1, 1, 1]) rotate([0, 0, 360/8/-2 - 45]) motor_mounting_bracket();
}
...
module assembled() {
  // drum assemblies
  translate([-case_width/2 + case_clearance + t + drum_width()/2, 0, 0]) for (i=[0:0]) {
    translate([i*(t + drum_width()), 0, 0]) {
      rotate([-360/20*5, 0, 0]) drum_assembly();
    }
  }

  // support struts for the main drums
  translate([-case_width/2 + case_clearance + t/2, 0, 0]) for (i=[0:1]) {
  ...
  //   rotate([-45, 0, 0]) translate([i * (drum_width() + t), center_distance(20, 8), 0])
  //     rotate([$t * -360, 0, 0]) 
  //     rotate([360/8 / 2, 0, 0]) 
  ...
  //   translate([x2 + x * (drum_width() + t), center_distance(20, 8), 0]) 
  ...
  //   rotate([-45, 0, 0]) 
  //     translate([0, center_distance(20, 8), 0]) 
  //       translate([(numeral_w/2 + t/2), 0, 0]) 
  //       rotate([$t * -360, 0, 0]) 
  //       rotate([360/8/2, 0, 0]) rotate([0, 90, 0]) drive_gear();
  // }
  ...
  // translate([-case_width/2 + case_clearance + drum_width() + t + 3.5*(drum_width()+t), 0, 0]) rotate([-45, 0, 0]) translate([0, center_distance(20, 8), 0]) 
